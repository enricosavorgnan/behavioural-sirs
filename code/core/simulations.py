...
class Simulations:
    ...
    def simulation_1(self, config_path : str) -> plt.Figure | None:
        """
        Simulation with different R₀ and θ parameters.
        """
        ...
        # Plot results
        show_cum_incid = config.get('show_cumulative_incidence', False)
        ...
        if config.get('plot_together', False):
            ...
            fig = Plots(show_cum_incid, show_params, show_title, True).plot_simulations(solutions=solutions, t_span=t_span, **params)
            return fig

        else:
            for i, solution in enumerate(solutions):
                params = {'image_path': f'./img/simulation_1/img_{i+1}/{len(solutions)}_r0_{r0s[i]}_theta_{thetas[i]}.png'}

                fig = Plots(show_cum_incid, show_params, show_title, True).plot_simulation(solution, t_span, **params)
                fig.show()
            return None
    ...
    def simulation_2_memory(self, config_path : str) -> plt.Figure | None:
        ...
        if config.get('plot_together', False):
            ...
            fig = Plots(show_cum_incid, show_params, show_title, True).plot_memory(solutions=solutions, t_span=t_span, **params)
            return fig
            ...
            for solution in solutions:
                ...
                fig = Plots(show_cum_incid, show_params, show_title, True).plot_memory(solution, t_span, **params)
                fig.show()
            return None
